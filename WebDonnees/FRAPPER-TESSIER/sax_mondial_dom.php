...
class MySaxHandler extends DefaultHandler {
	...
	//DOM ELEMENTS
	private $listepaysElem, $listeEspaceMaritimeElem, $racine;
	private $em;
	...
	{
	    //creation DOM de base
		$domImpl = new DOMImplementation();
		$docType = $domImpl->createDocumentType('em', '', 'em.dtd');
		global $em , $racine , $listepaysElem, $listeEspaceMaritimeElem;
	...
	**/
  	function endDocument() 
	{
        ...
        foreach($this->tableau_country as $objetCountry)
        {
        	...
        	foreach($this->tableau_mer as $objetMer)
        	{
	        	...
	        	foreach ($mer_country as $objetMerCountry) 
	        	{
	        		if($objetMerCountry==$id && !$already_wrote) // si le pays est cotoyés par cette mer et que l'on ne l'a pas déja écrit
		        	{
						...
						$paysElem->setAttribute('id-p', $id);
						$paysElem->setAttribute('nom-p', $nom);
						$paysElem->setAttribute('superficie', $super);
        	...
        	foreach($this->tableau_fleuve as $objetfleuve)
        	{
        		...
        		if($id_source == $id) // si le fleuve est la source du pays en question
        		{
        			...
        			//ajout attributs
        			$fleuveElem->setAttribute('id-f', $id_fleuve);
					...
					foreach ($id_pays_trav as $objetPaysTrav) 
					{
						...
						$parcourtElem->setAttribute('id-pays', $objetPaysTrav);
        ...
        foreach($this->tableau_mer as $objetMer)
        {
    		...
    		//attributs
    		$espaceElem->setAttribute('id-e', $id_mer);
    		$espaceElem->setAttribute('nom-e', $nom_mer);
    		$espaceElem->setAttribute('type', $type);

    		//ajout elements cotoie
        	foreach ($id_pays_trav as $objetPaysTrav) 
        	{
        		$cotoieElem = $em->createElement('cotoie');
				$cotoieElem->setAttribute('id-p', $objetPaysTrav);
